...
const formatNumber = (value: number) => value.toLocaleString("sr-RS");
          ...
          {rows.map((row) => (
            ...
            >
              ...
              <span className="adm__hint">
                {formatNumber(row.visitors)} posetilaca
              </span>
...
export function SaobracajPanel() {
      ...
      .catch(() => {
        if (active) {
          setData({
            ok: false,
            reason: "error",
            message: "Zahtev ka /api/admin/traffic nije uspeo.",
          });
        }
      })
      .finally(() => {
  ...
  const peak = data?.ok
    ? Math.max(1, ...data.daily.map((point) => point.pageviews))
    : 1;
  const labelEvery = data?.ok
          ...
          data.reason === "unconfigured" && (
            <div>
              <p className="adm__empty">
                Vercel Analytics nije povezan. Nedostaje:{" "}
                <strong>{data.missing.join(", ")}</strong>.
              </p>
              <p className="adm__hint">
                Dodaj promenljive iz ANALYTICS-SETUP.md i uradi redeploy.
              </p>
            </div>
          )}
          ...
          <>
            <div className="adm__stats">
              <div className="adm__stat">
                <strong>{formatNumber(data.totals.pageviews)}</strong>
              ...
              <div className="adm__stat">
                <strong>
                  {data.lifetime
                    ? formatNumber(data.lifetime.pageviews)
                    : "—"}
                </strong>
              ...
              <div className="adm__stat">
                <strong>
                  {data.lifetime
                    ? formatNumber(data.lifetime.visitors)
                    : "—"}
                </strong>
            ...
            {data.daily.length > 0 && (
              ...
              >
                {data.daily.map((point, index) => (
                  ...
                  >
                    <div
                      title={`${point.date} — ${formatNumber(point.pageviews)} pregleda, ${formatNumber(point.visitors)} posetilaca`}
                      style={{
                        width: "100%",
                        maxWidth: 48,
                        ...
                        minHeight: 3,
                        background:
                          "linear-gradient(180deg, var(--neon), rgba(46,107,255,.25))",
                        borderRadius: 4,
                      }}
                    />
                    <span
      ...
      {!loading && data?.ok && (
        <>
          ...
          <RowList
            title="Zemlje"
            rows={data.countries}
            empty="Nema podataka o zemljama."
          />
          <RowList
            title="Uređaji"
            rows={data.devices}
            empty="Nema podataka o uređajima."
          />
        </>
      )}
    </>
