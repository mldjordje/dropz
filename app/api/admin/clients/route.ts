  ...
  const rows = q
    ? await sql`
        SELECT u.id, u.name, u.email, u.avatar_url, u.admin_note, u.created_at, u.last_login_at,
          u.phone, u.gender,
          (u.phone IS NOT NULL AND u.birthday IS NOT NULL AND u.gender IN ('male', 'female')) AS profile_complete,
        ...
        FROM users u
        WHERE u.name ILIKE ${like} OR u.email ILIKE ${like} OR u.phone ILIKE ${like}
